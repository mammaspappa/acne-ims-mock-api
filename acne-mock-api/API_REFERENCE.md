...
| `GET` | `/api/v1/stakeholders/locations` | List all 29 locations. |
| `GET` | `/api/v1/stakeholders/suppliers` | List all 15 suppliers. |
| `GET` | `/api/v1/stakeholders/customers` | List persistent customers. Filters: `profile`, `country`, `tier`, `minOrders`, `sort` (totalSpent/totalOrders/lastOrderAt), `order`. |
| `GET` | `/api/v1/stakeholders/customers/:id` | Customer detail with recent order history. |
| `GET` | `/api/v1/stakeholders/customers/stats` | Aggregate stats per profile (count, avg orders, avg spent). |

**Customer profiles (5):** `VIC` (5%), `REGULAR` (20%), `RETURNING` (20%), `BARGAIN_HUNTER` (15%), `TOURIST` (40%)

**Customer tiers:** `BRONZE`, `SILVER`, `GOLD`, `PLATINUM` (only populated for VIC customers)

~3000 customers are seeded at startup with purchase history built up as the simulation runs. Every order is linked to a persistent customer via `customerId`, enabling lifetime value analysis, churn detection, and behavioral segmentation.
...
| `GET` | `/api/v1/admin/simulation` | Get simulation status (running, events generated, sim clock). |
| `GET` | `/api/v1/admin/simulation/log` | Paginated event log. Query: `?limit=50&offset=0` |
| `POST` | `/api/v1/admin/simulation/start` | Start simulation. Body: `{ passphrase, durationHours?, speedMultiplier?, autoScenarios? }` |
| `POST` | `/api/v1/admin/simulation/stop` | Stop simulation. Body: `{ passphrase }` |
| `GET` | `/api/v1/admin/simulation/scenarios` | List all 16 available scenario definitions. |
| `GET` | `/api/v1/admin/simulation/scenarios/active` | List currently active/resolved scenarios. |
...
Data is generated by running 30 simulated days through the simulation engine at startup. All records are cross-system consistent (every SO has payment events, every shipment has RFID scans, etc.).

| Entity | Count (approx.) | Notes |
|--------|-------|-------|
| Products | 54 | Realistic Acne Studios styles across 7 categories |
| SKUs | 729 | Products × colours × sizes |
| Locations | 29 | 3 warehouses (EU/NA/APAC) + 26 stores globally |
| Suppliers | 15 | Based on real Acne Studios supplier data (Italy, Portugal, China, Turkey, Romania, etc.) |
| Users | 20 | Across all 13 roles |
| Customers | ~3,000 | Persistent, with 5 behavior profiles (VIC/REGULAR/RETURNING/BARGAIN_HUNTER/TOURIST) |
| Purchase Orders | ~50-150 | Simulation-generated across full lifecycle |
| Sales Orders | ~1,000 | Linked to persistent customers, across channels |
| Stock Levels | ~7,300 | SKU/location combinations |
| Stock Movements | ~2,500 | Every stock change logged (sales, returns, transfers, damage) |
| Shipments | ~900 | Real carrier tracking numbers |
| SO↔PO Matches | ~50 | Confirmed, auto-confirmed, proposed, rejected, fulfilled |
| Season Drops, Forecasts, Anomalies | varies | Generated by the simulation engine |
